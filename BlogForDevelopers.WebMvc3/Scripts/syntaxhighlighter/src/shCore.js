...
var SyntaxHighlighter = function(ctorConfig) {
...
// Constants
	...
	/** Root URL for brush downloads; used if the entry point is boot().
	 *  Note: The default used here will not usually be suitable for anything
	 *  other than test code.
	 */
	var shRoot = "../syntaxhighlighter/" ;
...
var sh = {
	...
	config : {
		...
		/** If auto-loading brushes, wait this many milliseconds to allow
			additional brushes to arrive before calling doHighlights(). */
		brushTimer : 100
	},
...
//Newer verion		url							: /&lt;\w+:\/\/[\w-.\/?%&=@:;]*&gt;|\w+:\/\/[\w-.\/?%&=@:;]*/g,
	 ...
	 */
		...
		makeHighlighter: function(alias, alert)
		{
			...
			if (!brushClass) {
				if (sh.autoLoadBrushes) {
					// Load the brush automatically. When it loads and registers,
					// the highlighting will resume.
					sh.loadBrush(alias);
					return null;
				}
	...
	registerBrush: function(brushName, brushAliases, brushMethod)
	{
		...
		if (this.brushesToLoad) {
			if (!(--(this.brushesToLoad))) {
				// No more brushes need to be autoloaded; start highlighting.
				this.doHighlights();
			}
			else if (!this.brushTimerSet) {
				// We have at least one brush, but more are expected.
				// 
				// If we call doHighlights() for each individual brush, it can make the screen
				// appear to flicker as different code blocks are highlighted. Hence it might
				// be better to call doHighlights() after all brushes are loaded. But if we 
				// simply wait for all brush files to be loaded, we could get stuck if one or
				// more of the brush files don't exist, or otherwise fail to load. Hence we start
				// a timer, and call doHighlights() when the timer expires.
				//
				// It's OK if some brushes don't finish loading until after the timer expires; 
				// we'll just call doHighlights() (either directly, or via another timer)
				// when they're ready.

				window.setTimeout(function() {
					sh.brushTimerSet = false;
					if (sh.brushesToLoad) {
						sh.doHighlights();
					}
				}, this.config.brushTimer);
				this.brushTimerSet = true;
			}
			return;
		}

		this.doHighlights(brushName, brushAliases);
	},

	/**
	  * Used when SyntaxHighlighter is run in 'autoload' mode. 
	  * Remotely loads a CSS file.
	  *
	  * @param(href)		URL for the CSS file
      **/
	loadCSS : function(href)
	{
	    var cssNode = document.createElement('link');
		if (cssNode.setAttribute) {
	...
	/**
	  * Used when SyntaxHighlighter is run in 'autoload' mode. 
	  * Loads a CSS file that contains shCore.css + the CSS for the selected
	  * theme. (These are prepared by the build.pl script.)
      **/
	getTheme : function()
	{
		var theme = this.config.theme || "Default";

		if (theme === "none") {
			// Web page author requests not to load a theme file.
			return;
		}

		if (!/\.css$/i.test(theme)) {
			// Construct the full URL for the selected theme.
			theme = shRoot + "styles/shTheme" + theme + ".css";
			if (this.config.shVersion) {
				theme += "?ver=" + this.config.shVersion;
			}
		}
		this.loadCSS(theme);
	},
	...
	/**
	 * Maps a brush name to the script file that implements the brush.
	 * (The map data is generated by the build.pl script.)
	 */ 
	getBrushFile : (function()
	{
		var brushMap = {};

		// !!!MAPFILE!!! ( build.pl replaces this comment with the brush map data.)

		// Create a closure so that the brushMap need only be constructed once.
		return function(brushName) {


			if (/shBrush.*\.js/.test(brushName)) {
				// A full pathname was specified for the brushname; just return it as-is.
				return brushName;
			}

			var fileName = brushMap[brushName.toLowerCase()];
	...
	/**
	  * Load the script file for the specified brush, if it has not
	  * already been loaded.
	  */
   loadBrush : function(brushName)
   {
		if (!this.autoLoadBrushes) {
			// We're not set up for auto-loading brushes, probably because
			// SyntaxHighlighter.all() was called instead of SyntaxHighlighter.boot().
			return;
		}

		var brushFile = this.getBrushFile(brushName);
		var scriptElem = this.autoLoadedBrushes[brushFile];
		if (scriptElem) {
			return scriptElem;
		}

		scriptElem = document.createElement('script');
		this.autoLoadedBrushes[brushFile] = scriptElem;

		if (!/shBrush.*\.js/.test(brushName)) {
			// Construct the full pathname for this brush file.
			brushFile = shRoot + "scripts/shBrush" + brushFile + ".js";
			if (this.config.shVersion) {
				brushFile += "?ver=" + this.config.shVersion;
			}
		}

		scriptElem.setAttribute('type','text/javascript');
		scriptElem.setAttribute('src', brushFile);
	...
	/**
	 * ********************************************************************************
	 * NOTE: This method is still in test. Test before deploying; use at your own risk.
	 * ********************************************************************************
	 *
	 * An alternative to calling SyntaxHighlighter.highlight().
	 * When invoked through this function, the highlighter will
	 * automatically load brushes and CSS, rather than requiring
	 * them to be loaded within the HTML document.
	 *
	 * Note that for this to work, the rootURL parameter needs
	 * to point to the root of the tree where the SyntaxHighlighter
	 * scripts and styles directories can be found.
	 *
	 * @param rootURL	Points to the directory where the SyntaxHighlighter 
	 *					scripts and styles directories are found.
	 *
	 * @param config	Optional configuration parameters, overrriding
	 *					the values in SyntaxHighlighter.config
	 *
	 * @param defaults	Optional default values, overrriding
	 *					the values in SyntaxHighlighter.defaults
	 */
	boot : function(rootURL, config, defaults)
	{
		this.autoLoadBrushes = true;
		this.autoLoadedBrushes = {};
		this.brushesToLoad = 0;
		...
		var coreCSS = shRoot + "styles/shCore.css";
		this.loadCSS(coreCSS);
	 ...
	 * 
	 * @param {Object} jsParams		Optional parameters, which may be overridden by an element's 
	 * 								parameters.
	 * 
	...
	highlight : function(jsParams, element)
	{
		if (element) {
			...
			this.elementQueue.push({element: element, jsParams: jsParams});
		}
		else {
			// Highlight all appropriate elements.
			this.vars.ready = true;
		}

		if (ctorConfig) {
			sh.config = sh.utils.merge(sh.config, ctorConfig);

			// Do this merge only once.
			ctorConfig = null;
		}
	...
	doHighlights : function(forBrushName, forBrushAliases)
	{
		var highlightElem = function(target, params) 
		{
			...
			;
			// If this element uses HtmlScript, then we need the XML brush too.
			var needXMLBrush = (params['html-script'] === 'true' || sh.defaults['html-script'] === true);
			...
			if (forBrushName) {
				// If this call names a specific brush,
				// and this element requests a different brush, then return.
				// Allow for HtmlScript elements, which need the XML brush.
				if  ((brushName !== forBrushName && indexOf(forBrushAliases, brushName) < 0) &&
						!(needXMLBrush && /^xml$/i.test(forBrushName))) {
					return;
				}
			}

			if (needXMLBrush && !sh.brushes.Xml) {
				// We need the XML brush to be initialized; auto-load it if indicated.
				sh.loadBrush('Xml');
				return null;
			}
		...
		var itemParams, params, i, queue, item, target;
		...
		if ((queue = this.elementQueue)) {
			for (i = 0; i < queue.length; i++) {
				...
				target = item.element;
				itemParams = sh.utils.parseParams(target.className);

				//  itemParams take precedence over the passed-in params
				params = sh.utils.merge(item.jsParams, itemParams);
...
// Ensure that this handler is set up exactly once.
(function() {
	sh.utils.addEvent(window, 'resize',
					  	function(){
							sh.fixAllLineBackgrounds();
						}
					  );
})();

return sh;
// As we instantiate, pull in the configuration params that were passed to the boot loader (if any)
}(( window.SHLoader ? SHLoader.getConfig() : null) );
