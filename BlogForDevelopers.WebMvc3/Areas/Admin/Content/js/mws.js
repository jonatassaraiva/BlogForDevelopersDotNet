$(document).ready(function () {
	...
	var itensLocationPath = location.pathname.split('/');
	var locationHref = '';
	var showSubMenu = false;

	if (itensLocationPath.length >= 4) {
		locationHref = '/' + itensLocationPath[1] + '/' + itensLocationPath[2];
		if (itensLocationPath[3].length != 0) {
			locationHref += '/' + itensLocationPath[3];
			showSubMenu = true;
		}
	}
	else if (itensLocationPath.length == 3) {
		locationHref = '/' + itensLocationPath[1];
		if (itensLocationPath[2].length != 0) {
			locationHref += '/' + itensLocationPath[2];
			showSubMenu = true;
		}
	}
	else if (itensLocationPath.length == 2) {
		locationHref = '/' + itensLocationPath[1];
		
	}
	
	if(locationHref.toLocaleLowerCase() == "/admin/home/index" || locationHref.toLocaleLowerCase() == "/admin/home" ) {
		locationHref = "/admin";
	}

	
	$('div#mws-navigation ul li a[href="' + locationHref.toLocaleLowerCase() + '"]').parent().addClass("active");
	if (showSubMenu) {
		$('div#mws-navigation ul li a[href="' + locationHref.toLocaleLowerCase() + '"]').parent().parent()
